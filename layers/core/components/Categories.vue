...
<template>
  <div class="narrow-container pt-[40px] pb-[40px] md:pt-[80px] md:pb-[120px] space-y-6 md:space-y-10">
    ...
    <ul class="hidden md:grid grid-cols-3 gap-x-5 gap-y-10">
      <li v-for="(category, index) in items" :key="category.id">
        <a :href="category.slug">
          <NuxtImg :src="`/img/categories/${getCategoryImageByIndex(index)}.jpg`" class="w-full" :alt="category.name" />
    ...
    <SfScrollable buttons-placement="none" class="flex gap-4 md:hidden pb-8" :drag="true">
      <div v-for="(category, index) in items" :key="category.id" class="min-w-[200px] max-w-[300px]">
        <a :href="category.slug">
          <NuxtImg :src="`/img/categories/${getCategoryImageByIndex(index)}.jpg`" class="w-full" :alt="category.name" />
