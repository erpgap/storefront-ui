...
/** slug seguro para template/variante */
const slugFor = (pt: any) => pt?.firstVariant?.slug || pt?.slug || ''

/** normaliza estoque (algumas respostas usam `qty`) */
...
/** monta facets e dispara a busca */
      ...
      <div class="col-span-12 lg:col-span-8 xl:col-span-9">
        ...
        <div v-else-if="Array.isArray(productTemplateList) && productTemplateList.length > 0">
          ...
          <section class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 mt-8">
            <LazyUiProductCard
              v-for="pt in productTemplateList"
              :key="pt.id"
              :name="pt?.name || ''"
              loading="eager"
              :slug="slugFor(pt)"
              :image-url="$getImage(String(pt.image), 370, 370, String(pt.imageFilename))"
              :image-alt="pt?.name || ''"
              :regular-price="getRegularPrice(pt.firstVariant as any)"
          ...
          <LazyUiPagination
            v-if="(Number(totalItems) || 0) > 12"
            class="mt-5"
            :current-page="Number(route?.query?.page || 1)"
            :total-items="Number(totalItems) || 0"
            :page-size="12"
            :max-visible-pages="5"
          />
        </div>
