// plugins/fetch-logger.client.ts
...
export default defineNuxtPlugin((nuxtApp) => {
  // 1) Envolve o window.fetch (medição real de resBytes via clone().text())
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // tenta extrair opName/bytes de corpo (se for GraphQL)
    ...
    try {
      ...
      // mede tamanho da resposta (aproximado) — só se for JSON/text
      let resBytes = 0
      try {
      ...
      // contadores úteis (se for JSON GraphQL)
      ...
      console.log(
        `[client fetch ✓] ${gqlTag} → ${res.status} ${ms}ms` +
        (url ? ` (${url})` : ''),
        `req:${reqBytes}B`,
        `vars:${varsBytes}B`,
        `res:${resBytes}B`,
      ...
      const ms = Math.round(t1 - t0)
      const gqlTag = url.includes('/graphql') ? `GQL ${op}` : method
      console.error(`[client fetch ⨯] ${gqlTag} ${ms}ms`, err)
      throw err
    }
  }

  // 2) Envolve também o ofetch (nuxtApp.$fetch) — aqui o res é JSON já parseado
  const baseOfetch: any = nuxtApp.$fetch
  nuxtApp.$fetch = async (request: any, opts: any = {}) => {
    const url = typeof request === 'string' ? request : String(request?.url ?? '')
    const method = (opts?.method ?? 'GET').toUpperCase()
    const t0 = performance.now()

    const body = opts?.body ?? opts?.params
    const { op, varsBytes, reqBytes } = extractOp(body)

    try {
      const data = await baseOfetch(request, opts)
      const t1 = performance.now()
      const ms = Math.round(t1 - t0)

      // aproxima o tamanho serializando o JSON de volta
      let resBytes = 0
      try { resBytes = JSON.stringify(data).length } catch { /* ignore */ }

      const prods = data?.data?.products?.products?.length ?? data?.products?.products?.length ?? undefined
      const facets = data?.data?.products?.attributeValues?.length ?? data?.products?.attributeValues?.length ?? undefined
      const counts = (prods != null || facets != null) ? `items:${prods ?? '-'} facets:${facets ?? '-'}` : ''

      const gqlTag = url.includes('/graphql') ? `GQL ${op}` : method
      console.log(
        `[client $fetch ✓] ${gqlTag} → ${ms}ms` +
        (url ? ` (${url})` : ''),
        `req:${reqBytes}B`,
        `vars:${varsBytes}B`,
        `res~:${resBytes}B`,
